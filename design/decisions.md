...
- 2026-09-04 [완료] 대시보드 헤더 CPU 표시(사용자 요청) — os.getloadavg(stdlib, 의존성 0)로 load1/코어 수를 sys 필드로 서빙, 헤더에 "CPU x.x/N코어 (%)" 표기(70%↑ 주황·100%↑ 빨강). 첫 실측 1105%(load 110/10코어) — 머신 과부하 즉시 가시화
...
- 2026-09-04 [완료] 대시보드 UI 고도화 배치(사용자 요청 6건) — 2D 통신 그래프 제거(3D 전용), 트리 접기 토글 근본 수정(isCollapsed 스텁이 override를 무시하던 버그 — 이제 cmux 기본 접힘을 따름), 트리 모두 접기/펼치기 버튼, 분할 셀 단위 diff 렌더(3초 전면 재렌더 깜빡임 제거), ctx 경고 임계 80→60%, 세션 목록 툴팁 차단. 타일 미노출·구 토글 불능 신고는 낡은 pane으로 판명(헤드리스 스크린샷·DOM 덤프 실측)
- 2026-09-04 [완료] G1~G5 계측 공백 보강(loop-log #0001 발견분) — bus.to_event 확장: G1 tool=Skill에 skill명, G2 Agent/Task tool_input.subagent_type + 최상위 subagent_type 대체 키 수용, G3 이벤트명 정규화(sessionEnd→SessionEnd), G4 Notification message 120자, G5 uds 소켓 주소→레지스트리 이름 역해석(to_name). 전부 가산 필드라 스키마 v1 유지, 메타데이터 원칙 그대로(인자·본문 불저장 테스트 포함). 테스트 5건 추가(총 28)·라이브 검증. 잔여: G2 SubagentStop None의 실제 원인(훅 페이로드에 키 자체가 없을 가능성)은 다음 실측에서 확인, G5 실전 회신은 다음 dispatch 때
- 2026-09-04 [규칙] 재판정·분기 리뷰 임무를 LOOP_TEST(eharness-96, loop-eng)로 이관 — 리더 터미널은 /clear가 잦아 장기 임무 부적합(사용자 지시). LOOP_TEST가 loop-log에 보유 임무로 기록(접수 회신 확인)
- 2026-09-04 [규칙] 루프 실측 대장 design/loop-log.md 신설(사용자 요청: cctv 루프 엔지니어링 실측을 상시 기록할 기록자) — 판정 없이 L1 dispatch·L2 turn·L3 subagent·L4 probe·L5 collector 수치를 append. 기록 세션은 그룹 loop-eng 등록·자가 페이스 wakeup·읽기 전용 조회만(관측면 무개입). 드리프트 대조기 2026-09-18 재판정의 입력. 기준선 #0001에서 계측 공백 G1~G5 발견(Skill 이름 없음·agent_type None·sessionEnd 변형·Notification 종류 없음·uds 회신 역해석)
- 2026-09-06 [완료] init — 게이트 5 통과: why=Windows(WSL2) 설치 가이드 작성 중 `collect --install`이 launchd 전용이라 리눅스에서 수집기 상시 기동 불가 확인 + 수집기 죽은 상태를 다른 cctv 스킬 실패로만 뒤늦게 인지(사용자 요청 "cctv-init 명령어로 로컬 파이썬 서버를 켜서 관측"). what=~/.eharness/collect.pid(nohup 기동분만). how=skill:cctv-init(init.sh start|status|stop — launchd kickstart → systemd start → nohup 순). 무개입(settings·훅 불변). 테스트 2건(tests/test_init.py — 빈 포트 status exit1·실기동/멱등/stop). `wire init --install` 배선. Windows 설치 스크립트(~/Downloads/eharness-wsl-install.sh, 리포 밖)도 init 능력을 설치 목록에 포함
- 2026-09-06 [완료] Windows 네이티브 호환 패치(사용자 요청: PowerShell 설치 가이드) — 조사에서 치명 결함 발견: registry.alive의 `os.kill(pid, 0)`은 Windows에서 TerminateProcess라 수집기가 3초마다 모든 세션을 종료시킴 → nt 분기는 OpenProcess/GetExitCodeProcess 핸들 조회로만 판정. 그 외: 프로젝트 슬러그에 \\·: 처리(registry.slug), 훅 command 슬래시 경로(Windows 훅은 sh=Git Bash가 실행 — 공식 문서 확인), getuid 가드(terminal·collector debug), wire 검증의 bash 탐색(Git for Windows 기본 경로), init.sh의 EHARNESS_BIN 그대로 사용. 테스트 2건 추가(tests/test_registry.py, 총 32). 설치 스크립트는 리포 밖 ~/Downloads/eharness-windows-install.ps1(로그온 작업 스케줄러로 수집기 상시 기동, 외부 jq 자산은 미설치). Windows 실기 미검증 — 첫 실행 결과가 증거
- 2026-09-06 [완료] 설치기 편입 install/ (사용자 요청: 맥·윈도우 사용자가 같은 설치를 받도록) — 문제: 맥은 수동 구축·설치 스크립트 없음, WSL/Windows 스크립트는 리포 밖(~/Downloads), 외부 자산(~/.claude/hooks/*.sh·statusline.sh)은 리포 밖이라 세 경로가 같은 결과를 보장할 장치가 없었음. 최소 설계: install.sh(macOS·Linux·WSL2 — 상시 기동만 launchd/systemd/nohup 분기) + install.ps1(Windows — 로그온 작업, jq 는 winget 시도) + assets/ 정본(BSD/GNU stat 양쪽·osascript/notify-send 분기로 단일 사본) + merge_settings.py(멱등 병합, 단위 테스트). '같은 설치' 기준 = 검증 8항목 이름·순서 동일(테스트로 강제). 테스트 4건 추가(총 36). verify: 이 맥에서 install.sh 실기 실행. 미검증: Windows·WSL 실기
- 2026-09-06 [완료] launchd.install 에 kickstart 추가 — install.sh 실기에서 collect --install 직후 잡이 state=not running(never exited)으로 남아 검증 7/8. bootout→bootstrap 연속 호출의 경합. 재실행으로 8/8 확인

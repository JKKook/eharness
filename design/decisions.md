...
- 2026-09-04 [완료] 대시보드 브랜드 eharness → cctv(사용자 요청) — title·h1만. CLI 명령(eharness)·리포명은 우산 이름 유지(바인딩: 관측면=cctv). 오래 연 pane은 새로고침 필요
- 2026-09-04 [수정] dispatch 회신 규약 v2(사용자 설계) — 지연 판정을 고정 시간이 아닌 **선언 예상 + 유예 15분**으로: 팀원이 착수 보고(로컬/bg 구분+예상 시간)를 먼저 회신하고, bg 초과는 팀원이 wakeup으로 자가 감지해 중간 보고("[백그라운드] X 예상 초과"). 착수 보고 10분 무소식만 즉시 지연 신호. 장시간 bg(12h) 오탐 제거, 감지 주체=작업을 아는 팀원(수집기 무개입 유지). 잔여: 자가 보고는 soft — 미이행 목격 시 B1/M4 증거
